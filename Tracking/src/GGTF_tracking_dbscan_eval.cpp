#include "GGTF_tracking_dbscan_eval.hpp"
#include "dbscan.hpp"
#include <iostream>
#include <sstream>
#include <vector>
...
DECLARE_COMPONENT(GGTF_tracking_dbscan_eval)

GGTF_tracking_dbscan_eval::GGTF_tracking_dbscan_eval(const std::string& aName, ISvcLocator* aSvcLoc) : Gaudi::Algorithm(aName, aSvcLoc) {
    ...
    declareProperty("clustering_space", m_clustering_space, "Output embedding space");
    declareProperty("clustering_space_tracks", m_tracks_space, "Output tracks space");
  
}

GGTF_tracking_dbscan_eval::~GGTF_tracking_dbscan_eval() {}

StatusCode GGTF_tracking_dbscan_eval::initialize() { 
...
StatusCode GGTF_tracking_dbscan_eval::execute(const EventContext&) const {
    // Get the input collection with tracker hits
    std::cout << "Clustering algorithm: dbscan" << std::endl;
    ...
    const edm4hep::SimTrackerHitCollection* inputHits_VTXIB_sim = m_input_hits_VTXIB_sim.get();
    ...
    std::cout << "______________________________________________________" << std::endl;
    std::cout << "Tot hits: " << input_hits_CDC->size() + inputHits_VTXOB->size() + inputHits_VTXIB->size() + inputHits_VTXD->size() << std::endl;
    std::cout << "  " << std::endl;
    ...
    // Print the values of each row from output_vector
    auto output_cluster = m_clustering_space.createAndPut();
    for (int64_t i = 0; i < it; ++i) {
        for (int64_t j = 0; j < 4; ++j) {
            output_cluster->push_back(output_vector[i * 4 + j]);
    ...
    auto output_cluster_track = m_tracks_space.createAndPut();
    for (int64_t i = 0; i < it; ++i) {
        
        auto el = clustering.index({i}).item<double>();
    ...
    std::cout << "Number of hits per track" << std::endl;
    for (int i = 0; i < number_of_tracks; ++i) {
        ...
        std::cout << "track: " << id_of_track.item<int>() << "\tnumber of hits:" << number_of_hits << std::endl;
...
StatusCode GGTF_tracking_dbscan_eval::finalize() { return StatusCode::SUCCESS; }
