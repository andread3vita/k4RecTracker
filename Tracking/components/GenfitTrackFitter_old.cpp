                                                ...
                                                edm4hep::TrackCollection>(const edm4hep::TrackCollection&)>                                                                         
{
    ...
    StatusCode initialize() {     
        ...
        int debug_lvl_materials = (m_debug_lvl - 1 > 0) ? (m_debug_lvl - 1) : 0;
        genfit::MaterialEffects::getInstance()->setDebugLvl(debug_lvl_materials);           
                ...
                edm4hep::TrackCollection> operator()( const edm4hep::TrackCollection& tracks_input) const override                                                 
    {
        ...
        for (const auto& track : tracks_input)
        {
            ...
            if (track.getType() == 0) continue;                     // skip background
            num_tracks  +=1;
            ...
            for (int pdgCode : m_particleHypotesis)
            {
                ...
                track_interface.createGenFitTrack(1, m_debug_lvl);

                bool isFit = track_interface.fit(1, m_Beta_init, m_Beta_final, m_Beta_steps, m_Bz, m_debug_lvl); 

                // auto genTrack = track_interface.getTrack_genfit();
                // int numPoints = genTrack->getNumPoints();

                // for (int idx = 0; idx < numPoints; idx++)
                // {
                //     auto pt = genTrack->getPoint(idx);
                //     auto rawMeas = pt->getRawMeasurements();
                //     genfit::StateOnPlane state = genTrack->getFittedState(idx);
